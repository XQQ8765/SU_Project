...
// returns Monitoring:DBWC_GV_GlobalViewRoot
...
CAT = new EnableDebugLog4JLogger("script.loadGVData.su_6844")
...
def createDB2InstanceData (dbInstance, gvClusterData, monitoringAgent) {
    if (DEBUG_ENABLED) {
        ...
        CAT.debug("DB2 - createDB2InstanceData(): enter. ${paramsStr}")
    }
    ...
    if (DEBUG_ENABLED) {
        CAT.debug("DB2 - createDB2InstanceData(): exit.")
    }
}
...
def createDB2DatabaseData (currDBIns, instanceData, db2InstanceData, db2Instance) {
    if (DEBUG_ENABLED) {
        ...
        CAT.debug("DB2 - createDB2DatabaseData(): enter. ${paramsStr}")
    }
    ...
    CAT.trace("DB2 - createDB2DatabaseData(): after instanceActivity:${instanceActivity}, dbGeneralActivity:${dbGeneralActivity}.")
    ...
    */
    ...
    if (DEBUG_ENABLED) {
        CAT.debug("DB2 - createDB2DatabaseData(): exit.")
    }
...
def createDB2ClusterData (db2agent, db2InstanceMap, clusterData){
    if (DEBUG_ENABLED) {
        ...
        CAT.debug("DB2 - createDB2ClusterData(): enter. ${paramsStr}")
    }
    ...
    if (DEBUG_ENABLED) {
        CAT.debug("DB2 - createDB2ClusterData(): exit.")
    }
    return clusterData;
}
...
def createMSSQLClusterData (agentSql, instanceSql, clusterSql, GVclusterData, allHostTopologies) {
    if (DEBUG_ENABLED) {
        ...
        CAT.debug("MSSQL - createMSSQLClusterData(): enter. ${paramsStr}")
    }
    ...
    try {
        ...
        if (DEBUG_ENABLED) {
            CAT.debug("MSSQL - createMSSQLClusterData(): exit.")
        }
        return _clusterData;
    } catch (Exception e) {
...
def topologyBasicToString(topology) {
    ...
    return "(topologyTypeName:${topology?.topologyTypeName}, name:${topology?.name}, uniqueId:${topology?.uniqueId}, lastUpdated:${topology?.lastUpdated}, topologyObjectVersion:${topology?.topologyObjectVersion})"
}
...
def loadGVLastDataObject() {
    ...
    if (dbObservationTopology != null) {
        ...
        if(lastGVValue.size() == 1){
            ...
            DBWC_GV_GlobalViewDataObj.set("clustersGlobalView",lastGVValue.get(0).getValue());
...
def clustersGlobalView = gvData.get("clustersGlobalView");
...
if( unUpgraded != null ){
    println "Found un upgraded topologies: resetting global view data"
    clustersGlobalView = []
}
...
msg = "Un upgraded topologies check timed total: " + total001;
printDebugModeLog (msg, null) ;
...
for (int z=0; z < clustersGlobalView.size(); z++) {
    ...
    if (relevantInstanceName == null) {
        CAT.warn("extractTopologyKey is null, ignore clustersGlobalView ${clustersGlobalView.name}")
        continue;
    }
...
def returnValue = createEmptyGVData()
...
if (list != null && list.size() > 0) {
    ...
    returnValue.set("clustersGlobalView", list);
} else {
    printDebugModeLog("empty global view nodes list occurred", null);
    if (list != null) {
        printDebugModeLog("pushing empty list", null);
        def node = server.CanonicalFactory.createNode(fmsGvData, "clustersGlobalViewObs", list, sampledPeriod);
        dataService.publish(node);
        returnValue.set("clustersGlobalView", list);
    }
